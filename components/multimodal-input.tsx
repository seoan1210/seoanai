...
// PureMultimodalInput 컴포넌트: 메모이제이션(memo)을 위해 분리된 순수 컴포넌트야!
...
  // 로컬 저장소(localStorage)에 입력값을 저장하고 불러오는 훅!
...
      // 브라우저 DOM 값을 먼저 쓰고, 없으면 로컬 저장소 값을 사용해서 화면을 그려!
      const finalValue = domValue || localStorageInput || '';
      setInput(finalValue);
      adjustHeight();
    }
    // 화면이 처음 그려질 때 딱 한 번만 실행되도록 해!
...
        ]);
      } catch (error) {
        console.error('파일 업로드 중 오류가 발생했어요!', error);
      } finally {
        setUploadQueue([]);
      }
    },
...
      <AnimatePresence>
        {/* 스크롤이 맨 아래가 아니면 아래로 이동하는 버튼을 보여줘! */}
        {!isAtBottom && (
          <motion.div
...
      {/* 채팅 기록이 없거나 첨부된 파일이 없을 때 추천 액션들을 보여줘! */}
      {messages.length === 0 &&
...
          </PromptInputTools>
          {/* 모델이 응답 중일 때는 중지 버튼을, 아니면 전송 버튼을 보여줘! */}
          {status === 'submitted' ? (
...
// MultimodalInput 컴포넌트: PureMultimodalInput을 메모이제이션해서 불필요한 리렌더링을 막아줘!
...
// PureAttachmentsButton 컴포넌트: 첨부 버튼을 만드는 순수 컴포넌트야!
...
// PureModelSelectorCompact 컴포넌트: 모델 선택 드롭다운을 만드는 순수 컴포넌트야!
...
// PureStopButton 컴포넌트: 응답을 중지하는 버튼을 만드는 순수 컴포넌트야!
...
// PureSendButton 컴포넌트: 전송 버튼을 만드는 순수 컴포넌트야!
