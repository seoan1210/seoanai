...
import { useRouter } from 'next/navigation'; // 이 줄을 추가해!

export default function PrivacyModal() {
  const [open, setOpen] = useState(false);
  const [checked, setChecked] = useState(false);
  const router = useRouter(); // 이 줄을 추가해!
      ...
      >
        ...
        <div className="text-gray-800 mb-6 space-y-2">
          ...
          <ol className="list-decimal pl-5 space-y-1">
            ...
            <li>
              <strong>공지사함:</strong> 이미지 생성 모델 개발•생성 중
            </li>
          </ol>
        </div>
        ...
        {/* ⭐ 새로 추가된 코드: 개인정보 처리방침 페이지 링크 ⭐ */}
        <div className="text-gray-800 mb-4 text-sm mt-4">
          <p>자세한 내용은 
            <Link href="/privacy-policy" 
                  className="underline text-blue-600"
                  onClick={() => setOpen(false)} // 이 줄을 추가해!
            >개인정보 처리방침</Link> 전문을 확인해 주세요.</p>
        </div>
